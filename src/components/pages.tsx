...
import {
  AdminActions, AdminTable, AppShell, BroadcastView, Countdown, Hero, Leaderboard,
...
export function HomePage(){
 ...
 <section className="section"><div className="container"><div className="grid grid-4"><StatCard label="Current season" value="Season 05"/><StatCard label="Next event" value="Sep 12"/><StatCard label="Registration" value="Open"/><StatCard label="Entry fee" value="$0"/></div></div></section>
...
export function RoutePage({segments}:{segments:string[]}){
 ...
 if(root==="dashboard") return <Dashboard type={id}/>;
...
function Directory({type}:{type:string}){
 ...
 else if(type==="teams") content=<><div className="toolbar"><Search placeholder="Search teams, players, or tournaments…"/><Link className="btn btn-primary" href="/register/team">Register a team</Link></div><div className="grid grid-3">{teams.map((x,i)=><TeamCard name={x} index={i} key={x}/>)}</div></>;
...
function TournamentDetail({id,leaf}:{id:string;leaf?:string}){
 ...
 else if(leaf==="registration") content=<div className="card"><StatusBadge/><h3>Team registration</h3><p>Submit three starters, up to two substitutes, and rank snapshots. No EA credentials are collected.</p><Link className="btn btn-primary" href="/register/team">Open registration wizard</Link></div>;
 else content=<><div className="actions" style={{marginBottom:20}}>{["schedule","rules","registration","teams","leaderboard","matches","results","announcements"].map(section=><Link className="btn" href={`/tournaments/${id}/${section}`} key={section}>{section}</Link>)}</div><div className="grid grid-3"><StatCard label="Registered" value="17 / 20"/><StatCard label="Format" value="6 matches"/><StatCard label="Entry fee" value="$0"/></div><div className="grid grid-2" style={{marginTop:18}}><div className="card"><StatusBadge/><h3>Registration closes soon</h3><Countdown/><div className="actions"><Link className="btn btn-primary" href="/register/team">Register now</Link><Link className="btn" href={`/tournaments/${id}/matches`}>View matches</Link></div></div><QualificationProgress value={80}/></div><div style={{marginTop:50}}><SectionTitle eyebrow="Format" title="Tournament journey"/><TournamentTimeline/></div><div style={{marginTop:50}}><SectionTitle eyebrow="Results" title="Latest scores"/><ScoreTable/></div></>;
...
function Dashboard({type}:{type?:string}){
 const label=type==="team"?"Team workspace":type==="player"?"Player workspace":"Overview";
 return <AppShell><PageHeader eyebrow="Signed in · Demo mode" title={label} copy="Role-aware ALCL competition tools. Connected environments use Supabase Auth and row-level security."/><section className="container">
  <div className="grid grid-4"><StatCard label="Season rank" value="#04" change="↑ 2 places"/><StatCard label="Season points" value="111"/><StatCard label="Registration" value="Pending"/><StatCard label="Next event" value="Sep 12"/></div>
  <div className="grid grid-2" style={{marginTop:18}}><QualificationProgress value={72}/><div className="card"><h3>Qualification status</h3><p>Currently above the provisional top-12 qualification line.</p><Link className="btn" href="/championship">View qualification</Link></div></div>
  {type==="team"?<div className="card" style={{marginTop:18}}><h3>Registered roster & match lineup</h3><p>Manage three starters and up to two substitutes. Select exactly three eligible players for each match. Locked roster changes require organizer approval.</p><div className="actions"><button className="btn btn-primary">Edit unlocked roster</button><button className="btn">Set match lineup</button><button className="btn">Request roster change</button></div></div>:null}
  {type==="player"?<div className="card" style={{marginTop:18}}><h3>Player profile</h3><p>Review your role, self-reported rank snapshot, event history, wins, kills, achievements, and social links.</p><div className="actions"><button className="btn btn-primary">Edit profile</button><Link className="btn" href="/players/1">View public profile</Link></div></div>:null}
  <div className="grid grid-2" style={{marginTop:18}}><div className="card"><h3>Recent result</h3><p>Community Open · Match 6 · 23 match points</p><Link className="btn" href="/tournaments/community-open-1/results">View audited result</Link></div><div className="card"><h3>Notifications</h3><p>Registration changes requested · Roster lock approaching · Season standings updated</p><button className="btn">Mark all read</button></div></div>
 </section></AppShell>
}
